...
stdout_result="std.out"
stderr_result="std.err"
...
for inputfile in $2/*.in; do
  ...
  valgrind $valgrind_flags $prog >$stdout_result 2>$stderr_result  < $inputfile
  if [[ "$?" -eq 0 ]]; then
    echo "VALGRIND: OK"
  else
  ...
  diff $stdout_result $outputfile
  stdout_cmp=$?

  diff  $stderr_result $errorfile
  stderr_cmp=$?
